...
/* Initialize LEUART/USART and map LF to CRLF */
  ...
  BOARD_ledSet(ledMask);
	...
	printf("##EFR32 AES @ %u Hz %08X ARMCC:%u\n", 
	SystemCoreClock, 
	SCB->CPUID, 
	__ARMCC_VERSION);
	
	#ifdef __MICROLIB
	printf("With Microlib, FPU:%u\n", __FPU_USED);
	#else
	printf("With StandardLib, FPU:%u\n", __FPU_USED);
#endif

	//Begin Test
	lockTick = HAL_GetTick();
	uint8_t test_run_n;
	for(test_run_n=0; test_run_n<TEST_RUN_N; ++test_run_n)
	{
		/* 256 bit key */
		uint8_t key[] = {
			0x00, 0x01, 0x02, 0x03,
		...
		uint8_t mid[TEST_LEN]; // 128
		uint8_t out[TEST_LEN]; // 128
	...
	//	printf("\n");
		...
		for(uint32_t i=0; i<TEST_LEN/16; ++i)
		{	
			cipher(in+i*16, mid+i*16, w /* expanded key */);
		}
	...
	//	printf("\n");
		...
		for(uint32_t i=0; i<TEST_LEN/16; ++i)
		{	
			inv_cipher(mid+i*16 /* in */, out+i*16 /* out */, w /* expanded key */);
		}
		testRes = (0==memcmp(out, in, TEST_LEN));
	//	printf("result:\n");
	//	for (uint32_t i = 0; i < sizeof(in); i++) {
	//		printf("%02X ", in[i]);
	//	}
	...
	//End Test
	deltaTick = HAL_GetTick() - lockTick;
	printf("Res:%s RunTime:%u Ticks:%u\n", (testRes)?"OK":"Fail", test_run_n, deltaTick);
